  ...
  const {
    ...
    queryFn: async () => {
      ...
      const { data, error } = await queryKernel(
        `import cdm_tree_browser; (get_db_structure, get_table_schema, get_databases, get_tables, get_my_groups, get_namespace_prefix, using_mocks) = cdm_tree_browser.get_cdm_methods(); result = get_table_schema("${node.data?.database}", "${node.name}", return_json=True); result`,
        sessionContext
      );
...
export const berdlProvider: ITreeDataProvider<'tenant' | 'database' | 'table'> = {
  ...
  fetchRootNodes: async (sessionContext: SessionContext) => {
    const { data, error } = await queryKernel(
      'import cdm_tree_browser; (get_db_structure, get_table_schema, get_databases, get_tables, get_my_groups, get_namespace_prefix, using_mocks) = cdm_tree_browser.get_cdm_methods(); result = get_my_groups(return_json=True); result',
      sessionContext
    );
  ...
  fetchChildNodes: {
    tenant: async (
      ...
      const { data, error } = await queryKernel(
        `import cdm_tree_browser; import json; (get_db_structure, get_table_schema, get_databases, get_tables, get_my_groups, get_namespace_prefix, using_mocks) = cdm_tree_browser.get_cdm_methods(); databases = get_databases(use_hms=True, return_json=False, filter_by_namespace=False); prefix_response = get_namespace_prefix(tenant="${node.name}", return_json=False); result = {"databases": databases, "prefix": prefix_response}; json.dumps(result)`,
        sessionContext
      );
    ...
    database: async (
      ...
      const { data, error } = await queryKernel(
        `import cdm_tree_browser; (get_db_structure, get_table_schema, get_databases, get_tables, get_my_groups, get_namespace_prefix, using_mocks) = cdm_tree_browser.get_cdm_methods(); result = get_tables("${node.name}", use_hms=True, return_json=True); result`,
        sessionContext
      );
